...
// #define GRAMMARTOKENTYPE
        ...
        internal class /*union*/ GRAMMARSTYPE
        {
            internal uint flags;
            internal TypeDeclarator dec;
            internal List<TypeDeclarator> declist = new List<TypeDeclarator>();
            internal TypeSpecifiers spec;
            internal List<uint> ptrspec = new List<uint>();
            internal Datatype type;
            internal Enumerator enumer;
            internal List<Enumerator> vecenum = new List<Enumerator>();
            internal string str;
            internal ulong i;
        }
    ...
    // internal static int grammarparse();
    ...
    //    char nextchar;
    //    uint4 tok = GrammarToken.Token.badtoken;
    ...
    //if (mods.size() > 0)
    //    mod = mods[0];
    //if ((mod == (TypeModifier*)0) || (mod->getType() != TypeModifier.Modifier.function_mod))
    //    return false;
    ...
    internal static Datatype parse_type(TextReader s, out string name, Architecture glb)
        {
            CParse parser = new CParse(glb, 1000);

            if (!parser.parseStream(s, CParse.DocType.doc_parameter_declaration))
                throw new ParseError(parser.getError());
            List<TypeDeclarator> decls = parser.getResultDeclarations();
            if ((decls == (List<TypeDeclarator>)null) || (decls.Count == 0))
        ...
        internal static void parse_toseparator(TextReader s, out string name)
        {
            ...
            name = string.Empty;
            s.ReadSpaces();
            char tok = (char)s.Peek();

            while (char.IsLetterOrDigit(tok) || (tok == '_')) {
                tok = s.ReadMandatoryCharacter();
                name += tok;
                tok = (char)s.Peek();
            }
        }

        internal static Address parse_varnode(TextReader s, out int size, out Address pc,
            out uint uq, TypeFactory typegrp)
        {
            // Scan for a specific varnode
            int discard;

            Address loc = parse_machaddr(s, out size, typegrp);
            s.ReadSpaces();
            char tok = s.ReadMandatoryCharacter();
            if (tok != '(')
                throw new ParseError("Missing '('");
            s.ReadSpaces();
            tok = (char)s.Peek();
            // pc starts out as invalid
            pc = new Address();
            if (tok == 'i') {
                tok = s.ReadMandatoryCharacter();
            }
                ...
                pc = parse_machaddr(s, out discard, typegrp, true);
            }
            s.ReadSpaces();
            if ((char)s.Peek() == ':') {
                // Scan uniq
                tok = s.ReadMandatoryCharacter();
                s.ReadSpaces();
                s.ReadDecimalUnsignedInteger();
                // Assume uniq is in hex
                tok = s.ReadMandatoryCharacter();
                s.ReadSpaces();
                uq = s.ReadDecimalUnsignedIntegerWithKnownBase(
                    TextReaderExtensions.KnownBase.Hexadecimal);
            }
            else
                uq = uint.MaxValue;
            s.ReadSpaces();
...
//s.ReadSpaces() >> hex >> uq;       // Assume uniq is in hex
//return loc;
//}

        internal static Address parse_machaddr(TextReader s, out int defaultsize,
            TypeFactory typegrp, bool ignorecolon = false)
        {
            ...
            AddrSpaceManager manage = typegrp.getArch();
            s.ReadSpaces();
            char tok = (char)s.Peek();
            if (tok == '[') {
                tok = s.ReadMandatoryCharacter();
                // scan base address token
                parse_toseparator(s, out token);
                ...
                // Get the offset portion of the address
                parse_toseparator(s, out token);
                s.ReadSpaces();
                tok = s.ReadMandatoryCharacter();
                if (tok == ',') {
                    // Optional size specifier
                    size = s.ReadDecimalIntegerWithUnknownBase();
                    s.ReadSpaces();
                    tok = s.ReadMandatoryCharacter();
                }
                if (tok != ']')
                ...
                tok = s.ReadMandatoryCharacter();
                while (tok != '}')
                    // Scan to the matching curly brace
                    token += tok;
            }
            else {
                if (tok == '0') {
                ...
                if (b == (AddrSpace)null) {
                    token = s.ReadString();
                    throw new ParseError($"Bad address: {tok}{token}");
                }
                token = string.Empty;
                s.ReadSpaces();
                tok = (char)s.Peek();
                if (ignorecolon) {
                    while (char.IsLetterOrDigit(tok) || (tok == '_') || (tok == '+')) {
                        token += tok;
                        tok = s.ReadMandatoryCharacter();
                        tok = (char)s.Peek();
                    }
                    ...
                    while (char.IsLetterOrDigit(tok) || (tok == '_') || (tok == '+') || (tok == ':')) {
                        token += tok;
                        tok = s.ReadMandatoryCharacter();
                        tok = (char)s.Peek();
                    }
            ...
            Address res = new Address(b, 0);
            // Read the address of this particular type
            oversize = res.read(token);
            // oversize is "standard size"
            if (oversize == -1)
                throw new ParseError("Bad machine address");
            // If not overriden use standard
            defaultsize = (size == -1) ? oversize : size;
            return res;
        }
    }
}
