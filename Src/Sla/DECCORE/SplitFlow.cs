                ...
                switch (op.code()) {
                    ...
                    case OpCode.CPUI_SUBPIECE: {
                            if (outvn.isPrecisLo() || outvn.isPrecisHi())
                                // Do not split if we know value comes from double precision pieces
                                return false;
                            ...
                            else if (   (val == laneDescription.getSize(0))
                                     && (outvn.getSize() == laneDescription.getSize(1)))
                            {
                    ...
                    case OpCode.CPUI_INT_LEFT: {
                            Varnode tmpvn = op.getIn(1) ?? throw new ApplicationException();
                            if (!tmpvn.isConstant())
                                return false;
                            ulong val = tmpvn.getOffset();
                            if (val < laneDescription.getSize(1) * 8)
                                // Must obliterate all high bits
                                return false;
                            // Keep original shift
                            TransformOp rop = newPreexistingOp(2, OpCode.CPUI_INT_LEFT, op);
                            ...
                            opSetInput(rop, newConstant(op.getIn(1).getSize(), 0, op.getIn(1).getOffset()), 1);
                            break;
                        }
                    ...
                    case OpCode.CPUI_INT_RIGHT: {
                            ...
                            if (val < laneDescription.getSize(0) * 8)
                                return false;
                            OpCode extOpCode = (op.code() == OpCode.CPUI_INT_RIGHT)
